...
public class MongoRepository<TDocument, TId> : IRepository<TDocument, TId> where TDocument : class, IBaseEntity<TId>
{
    ...
    {
        return await _collection.Find(predicate).AnyAsync(cancellationToken: cancellationToken)!.ConfigureAwait(true);
    }
    ...
    {
        return await _collection.Find(predicate).ToListAsync(cancellationToken: cancellationToken)!.ConfigureAwait(true);
    }
    ...
    {
        return await _collection.AsQueryable().ToListAsync(cancellationToken).ConfigureAwait(true);
    }

    public async Task AddAsync(TDocument entity, CancellationToken cancellationToken = default)
    {
        await _collection.InsertOneAsync(entity, new InsertOneOptions(), cancellationToken).ConfigureAwait(true);
    }
    ...
    public async Task UpdateAsync(TDocument entity, CancellationToken cancellationToken = default)
    {
        ...
        _ = await _collection.ReplaceOneAsync(x => x.Id!.Equals(entity.Id), entity, cancellationToken: cancellationToken).ConfigureAwait(true);
    }
    ...
    {
        await _collection.DeleteOneAsync(d => d.Id!.Equals(id), cancellationToken).ConfigureAwait(true);
    }
