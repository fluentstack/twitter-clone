...
public static class ServiceCollectionExtensions
{
        ...
        where TContextImplementation : MongoDbContext, TContextService
    {
        ...
        if (string.IsNullOrEmpty(options.DatabaseName)) throw new ArgumentNullException(nameof(configuration));
        if (string.IsNullOrEmpty(options.ConnectionString)) throw new ArgumentNullException(nameof(configuration));
